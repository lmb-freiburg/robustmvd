import numpy as np  # TODO: tidy up
import torch
import pandas as pd
...
def pointwise_rel_ae(gt, pred, mask=None, output_scaling_factor=1.0):
    """Computes the pointwise relative-absolute-errors for a batch of predicted and ground truth depth maps.

    Args:
        gt: Ground truth depth map as torch tensor of shape Nx1HxW. Negative or 0 values are invalid and ignored.
        pred: Predicted depth map as torch tensor of shape Nx1HxW.
        mask: Torch tensor of shape Nx1HxW with numerical or boolean values for element weights or validity.
            For bool, False means invalid.
        output_scaling_factor: Scaling factor that is applied after computing the metrics (e.g. to get [%]).

    Returns:
        Torch tensor of shape Nx1HxW with pointwise relative-absolute-error values.
    """
    mask = (gt > 0).float() * mask if mask is not None else (gt > 0).float()

    e = pred - gt
    ae = torch.abs(e)
    rel_ae = torch.nan_to_num(ae / gt, nan=0, posinf=0, neginf=0)  # nan values are masked out anyways
    rel_ae *= mask
